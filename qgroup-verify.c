			   ...
			   u64 bytenr, u64 num_bytes, int meta_item)
{
	...
	if (flags & BTRFS_EXTENT_FLAG_TREE_BLOCK && !meta_item) {
		...
		/* skinny metadata */
		btrfs_item_key_to_cpu(ei_leaf, &tmp, slot);
		level = (int)tmp.offset;
	...
	if (level) {
		if (alloc_tree_block(bytenr, num_bytes, level))
			return ENOMEM;
	}
...
int qgroup_verify_all(struct btrfs_fs_info *info, int verbose)
{
	int ret;
	struct rb_node *node;
	...
	while (node) {
		...
		print_qgroup_difference(c, verbose);
		node = rb_next(node);
	}
