...
static int account_all_refs(int do_qgroups, u64 search_subvol)
{
	...
	return 0;
enomem:
	error("ERROR: Out of memory while accounting refs for qgroups!\n");
	return -ENOMEM;
}
			      ...
			      struct qgroup_count *count)
{
	...
	if (ret) {
		fprintf(stderr, "ERROR: Could not find disk item for qgroup "
			"%llu/%llu.\n", btrfs_qgroup_level(count->qgroupid),
			btrfs_qgroup_subvid(count->qgroupid));
		if (ret > 0)
			ret = -ENOENT;
...
static int repair_qgroup_status(struct btrfs_fs_info *info)
{
	...
	ret = btrfs_search_slot(trans, root, &key, path, 0, 1);
	if (ret) {
		fprintf(stderr, "ERROR: Could not find qgroup status!\n");
		if (ret > 0)
			ret = -ENOENT;
		goto out;
	}
